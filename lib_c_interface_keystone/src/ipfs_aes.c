...
// 传入的pt是指向一个ss.size大小的切片空间[]byte
// full := pool.Get(int(ss.size))
// pt就是(*C.void)(unsafe.Pointer(&full[0]))
//
int encrypt(void* pt, int ptLen, void* ct) {
    ...
    int align_ptLen = (ptLen + 0xf) &~0xf;
    memset(pt + ptLen, 0, align_ptLen - ptLen);
    for (size_t i = 0; i < align_ptLen; i+=16)
    {
        aes_encrypt((u32*)rk1, (u8*)(pt + i), (u8*)(ct + i));
    }
...
int decrypt(void* ct, int ctLen, void* pt) {
    ...
    int align_ctLen = (ctLen + 0xf) &~0xf;
    for (size_t i = 0; i < align_ctLen; i+=16)
    {
        aes_decrypt((u32*)rk2, (u8*)(ct + i), (u8*)(pt + i));
    }
